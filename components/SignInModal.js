...
import { SignIn, FacebookLogin, GoogleLogin } from "../utils/auth";
...
export default function SignInModal({ handleOpen, open, setOpenLogin }) {
      ...
      <Dialog open={open} handler={handleOpen} animate={{
        ...
        <DialogBody divider className="flex flex-col flex-grow gap-y-4 items-center">
          <form className="flex flex-col flex-grow gap-y-4">
            <Input onChange={(e)=>setUsername(e.target.value)} name="username" value={username} className="text-2xl" label="Username" icon={<FaUserAlt />} />
            <Input onChange={(e)=>setPassword(e.target.value)} name="password" value={password} className="text-2xl" label="Password" type="password" icon={<MdPassword />} />
          </form>
        ...
        <DialogFooter className="flex gap-x-2">
          ...
          <Button variant="gradient" color="green" onClick={handleSignIn}>
            <span>Sign in</span>
          </Button>
        </DialogFooter>
      </Dialog>
